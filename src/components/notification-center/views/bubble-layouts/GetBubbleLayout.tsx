import { AchievementNotificationBubbleItem, MentionNotificationBubbleItem, NotificationBubbleItem, NotificationBubbleType } from '../../../../api';
...
export const GetBubbleLayout = (item: NotificationBubbleItem, onClose: () => void) => {
    ...
    switch (item.notificationType) {
        case NotificationBubbleType.ACHIEVEMENT:
            // A level-up carries its badge and gets the wear button of the badge bubble; a plain
            // achievement bubble (no badge item) keeps the dedicated level-up layout.
            if (item instanceof AchievementNotificationBubbleItem) return <NotificationBadgeReceivedBubbleView key={item.id} {...timedProps} />;
