...
export const GuideToolView: FC<{}> = (props) => {
    ...
    const onGuideToolEvent = useCallback(
        (event: GuideToolEvent) => {
            switch (event.type) {
            ...
            switch (action) {
                ...
                case 'toggle_duty':
                    if (!isHandlingBullyReports && !isHandlingGuideRequests && !isHandlingHelpRequests) {
                        simpleAlert(
                            LocalizeText('guide.help.guide.tool.noqueueselected.message'),
                            null,
                            null,
                            null,
                            LocalizeText('guide.help.guide.tool.noqueueselected.caption'),
                            null
                        );
                        return;
                    }
                    ...
                    setIsOnDuty((v) => {
                        SendMessageComposer(
                            new GuideSessionOnDutyUpdateMessageComposer(
                                !v,
                                v ? false : isHandlingGuideRequests,
                                v ? false : isHandlingHelpRequests,
                                v ? false : isHandlingBullyReports
                            )
                        );
                        return !v;
                    });
                    ...
                    return;
                case 'forum_link':
                    const url: string = GetConfigurationValue<string>('group.homepage.url', '').replace(
                        '%groupid%',
                        GetConfigurationValue<string>('guide.help.alpha.groupid', '0')
                    );
    ...
    return (
        <OctaneCardView className={`octane-guide-tool${chatReviewClassName}`} theme="primary-slim">
            ...
            <OctaneCardContentView className="text-black">
                ...
                {sessionState === GuideSessionState.USER_CREATE && <GuideToolUserCreateRequestView setUserRequest={setUserRequest} userRequest={userRequest} />}
                ...
                {sessionState === GuideSessionState.USER_FEEDBACK && <GuideToolUserFeedbackView userName={ongoingUsername} />}
